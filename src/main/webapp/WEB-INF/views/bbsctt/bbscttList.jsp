					...
					<div class="list">
						...
						<div class="transactionTable">
							<table class="tb-1">
								...
								<c:forEach var="bbsctt" items="${LIST}" varStatus="status">
									<input id="bbscttNo" type="hidden" value="${bbsctt.bbscttNo}">
									<tr>
										<td>${bbsctt.bbscttNo}</td>
										<td class="bst-sj"><a href="bbscttDetail.do?BBSCTT_NO=${bbsctt.bbscttNo}" class="bbscn-1">${bbsctt.bbscttSj}</a>
											<c:if test="${COMENT_CNT[status.index] ne 0}">
												<a href="#" class="bbscn-2">[${COMENT_CNT[status.index]}]</a>
											</c:if>
										</td>
										<td class="bst-nc">${MBER_LIST[status.index].mberNcnm}</td>
