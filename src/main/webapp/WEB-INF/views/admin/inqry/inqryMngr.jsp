...
$(function(){
	...
	// 상세보기 버튼 처리
	$("input[name='insertMngrInqryFormBtn']").each(function(i, e){
		$(this).click(function(){
			$("input[name='mngrInqryNo']").val($(".mngrInqryNo").eq(i+1).text());
	...
	// 답변보기 버튼 처리
	$("input[name='mngrAnswerDetailBtn']").each(function(i, e){
		$(this).click(function(){
			$("input[name='mngrInqryNo']").val($(".mngrInqryNo").eq(i+1).text());
			$("input[name='start']").val(conditionParam.start);
			$("input[name='cnt']").val(conditionParam.cnt);
	...
	page("mngrInqry", total, conditionParam, pageHtml);
	            ...
	            <div class="box-wrap">
	            	...
	            	<div class="search box">
	            		...
	            		<div class="search_btn s_box">
	            			<input type="submit" name="searchBtn" value="검색" formaction="mngrInqryCondition.do"> 
	            		</div>
	            	</div>
	            	<div class="list box">
	            			...
	            			<div class="l_row l_box">
	            			    ...
	            			    <div class="mberEmail l_column">
	            			        ...
	            			        <c:if test="${fn:length(textMberEmail) > 20}">
	            			            ${fn:substring(inqryMngr.mberEmail, 0, 18)}..
	            			        </c:if>
	            			        <c:if test="${fn:length(textMberEmail) <= 20}">
	            			            ${inqryMngr.mberEmail}
	            			        </c:if>
	            			    ...
	            			    <div class="inqrySj l_column">
	            			        ...
	            			        <c:if test="${fn:length(textInqrySj) > 20}">
	            			            ${fn:substring(textInqrySj, 0, 18)}..
	            			        </c:if>
	            			        <c:if test="${fn:length(textInqrySj) <= 20}">
	            			            ${inqryMngr.mngrInqryDto.inqrySj}
	            			        </c:if>
	            			    </div>
	            			    <div class="inqryDt l_column">
	            			        <fmt:formatDate value="${inqryMngr.mngrInqryDto.inqryDt}" pattern="yyyy-MM-dd"/>
	            			    </div>
	            			    <div class="etc2 l_column">
	            			        <input type="submit" name="mngrInqryDetail" value="상세보기" formaction="mngrInqryDetail.do">
	            			        <c:if test="${inqryMngr.mngrInqryDto.mngrAnswerNo == null}">
	            			            <input type="submit" name="insertMngrInqryFormBtn" value="답변등록" formaction="insertMngrInqryFrom.do">
	            			        </c:if>
	            			        <c:if test="${inqryMngr.mngrInqryDto.mngrAnswerNo != null}">
	            			            <input type="submit" name="mngrAnswerDetailBtn" value="답변보기" formaction="mngrInqryDetail.do">
	            			        </c:if>
	            			    </div>
	            		    </div>
	            		</c:forEach>
	            		<div>
	            		    <input type="hidden" name="mngrInqryNo" value="">
							<input type="hidden" name="isAnswer" value="">
